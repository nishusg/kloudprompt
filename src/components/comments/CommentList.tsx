...
import { Button, CircularProgress, Box, Typography, Stack, TextField, Paper, Alert } from "@mui/material";
import { getComments, addCommentToPrompt } from "../../services/CommentService";
...
return (
    ...
    {/* Add Comment */}
    {isAuthenticated ? (
        <Paper sx={{ p: 2, borderRadius: 2, bgcolor: '#1e1e1e', borderColor: '#333', mb: 2 }} variant="outlined">
        <TextField
            fullWidth
            multiline
            rows={3}
        ...
        />
        <Button variant="contained" color='primary' onClick={handleAddComment} disabled={isCommenting}>
            {isCommenting ? <CircularProgress size={24} sx={{ color: '#000' }} /> : 'Post Comment'}
        </Button>
        </Paper>
    ) : (
        <Alert severity="info" sx={{ bgcolor: '#1e1e1e', color: '#fff', border: '1px solid #333', mb: 2 }}>
    ...
    {hasMore && !loading && comments.length > 0 && (
        <Button onClick={handleLoadMore} sx={{ display: 'block', mx: 'auto', mb: 2 }}>
        Load More
        </Button>
    )}
    </Box>
